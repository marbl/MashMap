...
namespace skch
{
  ...
  class Sketch
  {
    ...
    void build()
    {
      ...
      std::vector<MI_Type> outputBuffer(this->inputBufferSize);
      ...
      for(const auto &fileName : param.refSequences)
      {
        ...
        while ((len = kseq_read(seq)) >= 0) 
        {
          ...
          else
          {
            ...
            {
              this->processBuildWorkInParallel(pendingWorkItems, inputBuffer, outputBuffer);
    ...
    /**
     * @brief                           schedule build tasks inside openmp runtime system
     * @param[in]   pendingWorkItems    count of input tasks
     * @param[in]   inputBuffer         reference sequences
     * @param[in]   outputBuffer        temporary container for threads to save minimizers
     */
    template <typename Vec1, typename Vec2>
      void processBuildWorkInParallel(int pendingWorkItems, Vec1 &inputBuffer, Vec2 &outputBuffer)
      {
#pragma omp parallel for schedule(static, 1) 
        for(int i=0; i < pendingWorkItems; i++)
        {
          skch::CommonFunc::addMinimizers(outputBuffer[i], &inputBuffer[i].seq[0u], inputBuffer[i].len, param.kmerSize, param.windowSize, param.alphabetSize, inputBuffer[i].seqCounter);
        }

        //Process output sequentially
        for(int i=0; i < pendingWorkItems; i++)
        {
          this->buildHandleThreadOutput(outputBuffer[i]);
        }
      }
